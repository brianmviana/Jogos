...
sfRenderWindow * criar_janela(int width, int height){
    ...
    sfRenderWindow * _janela = sfRenderWindow_create(mode," (= kill alins =) ", sfResize | sfClose, NULL);
...
void csfml_sprite_setSize(sfSprite *sprite, float largura, float altura){
    ...
    sfSprite_setScale(sprite,(sfVector2f){largura / rect.width, altura / rect.height});
}
